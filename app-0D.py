...
if st.button("🔍 Calcular Esforço no Poste", type="primary", use_container_width=True):
    try:
        ...
        if tem_pr:
            ...
            # S_arrasto para condutores APENAS (PR rompeu)
            S_arrasto_sem_pr = 0
            for i in range(len(K_fases)):
                if i < 3:
                    S_arrasto_sem_pr += D1 * K_fases[i] * cabos_por_fase
                else:
                    if is_duplo:
                        S_arrasto_sem_pr += D2 * K_fases[i] * cabos_por_fase
            
            # Ângulo crítico do vento (sem o PR)
            ang_vento_pr_rompido_critico, _ = encontrar_angulo_critico_vento(
                ...
                S_arrasto_sem_pr, pressao_vento_reduzida, vao_re, vao_vante, F_vento_poste_reduzida
            )
            
            # Força de arrasto nos cabos (sem PR)
