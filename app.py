...
# ===================================================
# FUNÇÃO PARA CARREGAR ESTRUTURAS
# ===================================================
@st.cache_data
def carregar_estruturas():
    """Carrega a planilha de estruturas do GitHub"""
    try:
        # Tentar carregar do arquivo local (Streamlit Cloud)
        if os.path.exists("estruturas.xlsx"):
            df = pd.read_excel("estruturas.xlsx", header=None)
        else:
            # Tentar carregar do GitHub raw
            url = "https://raw.githubusercontent.com/elizeuclp-cpu/vhpt-app/main/estruturas.xlsx"
            df = pd.read_excel(url, header=None)
        
        # Mapear colunas (índices 0-based)
        estruturas = []
        for idx, row in df.iterrows():
            ...
            if liberacao == "SIM":
                estrutura = {
                    ...
                    'tensao': row[3] if pd.notna(row[3]) else None,
        ...
        st.warning(f"Não foi possível carregar a planilha de estruturas: {e}")
        return []
...
# CARREGAR ESTRUTURAS
# ===================================================
estruturas_disponiveis = carregar_estruturas()
nomes_estruturas = [e['nome'] for e in estruturas_disponiveis]
...
with col1:
    ...
    # Seleção de estrutura (se houver)
    modo_entrada = st.radio("Modo de entrada", ["Selecionar estrutura", "Manual"], index=0)
    
    if modo_entrada == "Selecionar estrutura" and nomes_estruturas:
        estrutura_selecionada = st.selectbox("Selecionar estrutura", nomes_estruturas)
        # Buscar a estrutura selecionada
        estrutura_atual = next((e for e in estruturas_disponiveis if e['nome'] == estrutura_selecionada), None)
        
        if estrutura_atual:
            tipo_poste = estrutura_atual['tipo_poste']
            fixacao = estrutura_atual['fixacao']
            locacao = estrutura_atual['locacao']
            is_duplo = (estrutura_atual['circuito'] == "Circuito Duplo")
            tem_pr = estrutura_atual['tem_pr']
            dist_f1 = estrutura_atual['dist_f1']
            dist_f2 = estrutura_atual['dist_f2']
            dist_f3 = estrutura_atual['dist_f3']
            dist_f4 = estrutura_atual['dist_f4']
            dist_f5 = estrutura_atual['dist_f5']
            dist_f6 = estrutura_atual['dist_f6']
            dist_pr = estrutura_atual['dist_pr']
            
            # Definir deflexão padrão conforme fixação
            if fixacao == "suspensao":
                deflexao = 0
            else:
                deflexao = 30  # valor padrão para ancoragem
            
            st.success(f"✅ Estrutura carregada: {estrutura_selecionada}")
        else:
    ...
    if modo_entrada == "Selecionar estrutura" and estrutura_atual:
        num_circuitos = "Circuito Duplo" if is_duplo else "Circuito Simples"
        st.info(f"Circuitos: {num_circuitos}")
    else:
...
with col2:
    ...
    with col_f1:
        dist_f1 = st.number_input("Fase 1 (m)", value=float(dist_f1), step=0.05)
    with col_f2:
        dist_f2 = st.number_input("Fase 2 (m)", value=float(dist_f2), step=0.05)
    with col_f3:
        dist_f3 = st.number_input("Fase 3 (m)", value=float(dist_f3), step=0.05)
    
    if is_duplo:
        st.markdown("**Circuito 2:**")
        col_f4, col_f5, col_f6 = st.columns(3)
        with col_f4:
            dist_f4 = st.number_input("Fase 4 (m)", value=float(dist_f4), step=0.05)
        with col_f5:
            dist_f5 = st.number_input("Fase 5 (m)", value=float(dist_f5), step=0.05)
        with col_f6:
            dist_f6 = st.number_input("Fase 6 (m)", value=float(dist_f6), step=0.05)
    ...
    if modo_entrada == "Selecionar estrutura" and estrutura_atual:
        tem_pr_check = tem_pr
        st.info(f"Cabo PR: {'Sim' if tem_pr else 'Não'}")
    else:
        tem_pr_check = st.checkbox("Possui cabo para-raios?")
    ...
    if tem_pr_check:
        if modo_entrada == "Selecionar estrutura" and estrutura_atual:
            ...
            dist_pr_val = st.number_input("Distância PR ao topo (m)", value=0.20, step=0.05)
...
if st.button("🔍 Calcular Esforço no Poste", type="primary", use_container_width=True):
    try:
        ...
        # Coeficientes K
        dist_pr = dist_pr_val if tem_pr_check else None
        ...
        # ===================================================
        # HIPÓTESE 4: CABO CONDUTOR ROMPIDO (simplificada)
        # ===================================================
        ...
        F_re_rompido_A = sum([K_fases[i] * T_re_rompido_A[i] for i in range(num_fases)]) + (K_pr * T_pr_re_rompido if tem_pr_check else 0)
        F_vante_rompido_A = sum([K_fases[i] * T_vante_rompido_A[i] for i in range(num_fases)]) + (K_pr * T_pr_vante_rompido if tem_pr_check else 0)
        ...
        F_re_rompido_B = sum([K_fases[i] * T_re_rompido_B[i] for i in range(num_fases)]) + (K_pr * T_pr_re_rompido if tem_pr_check else 0)
        F_vante_rompido_B = sum([K_fases[i] * T_vante_rompido_B[i] for i in range(num_fases)]) + (K_pr * T_pr_vante_rompido if tem_pr_check else 0)
        ...
        # ===================================================
        # EXIBIR RESULTADOS (resumido para caber na resposta)
        # ===================================================
        st.markdown("---")
        ...
        # Tabela resumida
        dados_tabela = [
            {'Hipótese': 'EDS', 'Esforço Final (daN)': f"{R_eds_daN:.1f}", 'Poste': f"{poste_eds} daN"},
            {'Hipótese': 'TEMP MÍNIMA', 'Esforço Final (daN)': f"{R_min_daN:.1f}", 'Poste': f"{poste_min} daN"},
            {'Hipótese': 'VENTO MÁXIMO', 'Esforço Final (daN)': f"{R_vento_daN:.1f}", 'Poste': f"{poste_vento} daN"},
            {'Hipótese': 'CABO ROMPIDO', 'Esforço Final (daN)': f"{R_rompido_daN:.1f}", 'Poste': f"{poste_rompido} daN"},
        ]
        ...
        with col_conc1:
            st.metric("Maior esforço", f"{max(R_eds_daN, R_min_daN, R_vento_daN, R_rompido_daN):.1f} daN")
        with col_conc2:
