...
# ===================================================
# FUNÇÃO PARA CARREGAR ESTRUTURAS DO ARQUIVO
# ===================================================
@st.cache_data
...
def calcular_forca_vento_poste(tipo_poste, altura_poste, pressao_vento):
    """Calcula a força de vento no poste (kgf) aplicada a 20 cm do topo"""
    h = altura_poste
    
    if tipo_poste == "Duplo T":
        numerador = h**2 * (0.34524 + 0.01134 * h) * (1.1592 + 0.0252 * h)
        denominador = (2.3016 + 0.0756 * h) * (0.9 * h - 0.8)
        F_vento = 1.62 * pressao_vento * (numerador / denominador)
    else:  # Circular
        numerador = h**2 * (0.42345 + 0.006075 * h) * (1.416 + 0.0135 * h)
        denominador = (2.823 + 0.0405 * h) * (0.9 * h - 0.8)
        F_vento = 1.08 * pressao_vento * (numerador / denominador)
    
    return F_vento

def encontrar_angulo_critico_vento(F_re, F_vante, ang_re, ang_vante, S_arrasto, P_vento, L_re, L_vante, F_vento_poste):
    melhor_angulo = 90
    maior_resultante = 0
    ...
    for ang_vento in range(0, 181, 1):
        ...
        F_vento_cabos_mag = (P_vento/2) * (L_re * seno_re + L_vante * seno_vante) * S_arrasto
        F_vento_total_mag = F_vento_cabos_mag + F_vento_poste
        F_vento_total = forca_complexa(F_vento_total_mag, ang_vento)
        R_total = abs(F_cabos + F_vento_total)
...
# ===================================================
# CARREGAR ESTRUTURAS (SE ARQUIVO EXISTIR)
# ===================================================
...
with col1:
    ...
    if tem_estruturas:
        ...
        tipo_estrutura_opcional = st.selectbox(
...
with col2:
    ...
    if tem_pr:
        tipo_pr = st.selectbox("Tipo de PR", ["EHS 5/16\"", "EHS 3/8\""], index=0)
...
if st.button("🔍 Calcular Esforço no Poste", type="primary", use_container_width=True):
    try:
        ...
        ang_vento_critico, _ = encontrar_angulo_critico_vento(
            ...
            S_arrasto, pressao_vento, vao_re, vao_vante, F_vento_poste_normal)
        
        seno_re = abs(np.sin(np.radians(ang_vento_critico - ang_re)))
        seno_vante = abs(np.sin(np.radians(ang_vento_critico - ang_vante)))
        F_vento_cabos_mag = (pressao_vento/2) * (vao_re * seno_re + vao_vante * seno_vante) * S_arrasto
        F_vento_mag = F_vento_cabos_mag + F_vento_poste_normal
        ...
        ang_vento_rompido_critico, _ = encontrar_angulo_critico_vento(
            ...
            S_arrasto_reduzido, pressao_vento_reduzida, vao_re, vao_vante, F_vento_poste_reduzida)
        ...
        F_vento_cabos_rompido_mag = (pressao_vento_reduzida/2) * (vao_re * seno_re_rompido + vao_vante * seno_vante_rompido) * S_arrasto_reduzido
        F_vento_rompido_mag = F_vento_cabos_rompido_mag + F_vento_poste_reduzida
        ...
        # ===================================================
        # HIPÓTESE 5: CABO PARA-RAIOS ROMPIDO (CORRIGIDA)
        # ===================================================
        if tem_pr:
            ...
            # Trações do PR com vento reduzido
            T_pr_re_rompido_pr = mudanca_estado(
                T_pr_re_eds, temp_eds, temp_vento, 
                peso_pr, peso_composto_pr_red, vao_re, E_pr, S_pr, alpha_pr
            )
            T_pr_vante_rompido_pr = mudanca_estado(
                T_pr_vante_eds, temp_eds, temp_vento, 
                peso_pr, peso_composto_pr_red, vao_vante, E_pr, S_pr, alpha_pr
            )
            
            # Trações dos condutores com vento reduzido
            fv_max_red = D1 * pressao_vento_reduzida
            peso_composto_red = np.sqrt(peso1**2 + fv_max_red**2)
            
            T_circ1_re_rompido_pr = mudanca_estado(
                T_circ1_re_eds, temp_eds, temp_vento, 
                peso1, peso_composto_red, vao_re, E1, S1, alpha1
            )
            T_circ1_vante_rompido_pr = mudanca_estado(
                T_circ1_vante_eds, temp_eds, temp_vento, 
                peso1, peso_composto_red, vao_vante, E1, S1, alpha1
            )
            
            if is_duplo:
                fv_max_red2 = D2 * pressao_vento_reduzida
                peso_composto_red2 = np.sqrt(peso2**2 + fv_max_red2**2)
                T_circ2_re_rompido_pr = mudanca_estado(
                    T_circ2_re_eds, temp_eds, temp_vento, 
                    peso2, peso_composto_red2, vao_re, E2, S2, alpha2
                )
                T_circ2_vante_rompido_pr = mudanca_estado(
                    T_circ2_vante_eds, temp_eds, temp_vento, 
                    peso2, peso_composto_red2, vao_vante, E2, S2, alpha2
                )
            else:
                T_circ2_re_rompido_pr = T_circ2_vante_rompido_pr = 0
            
            # Cenário A: rompe o PR no ré
            F_re_rompido_pr_A = (
            ...
            F_vante_rompido_pr_A = (
                ...
                K_pr * T_pr_vante_rompido_pr
            )
            
            # Cenário B: rompe o PR no vante
            F_re_rompido_pr_B = (
                K_circ1 * T_circ1_re_rompido_pr + 
                K_circ2 * T_circ2_re_rompido_pr + 
                K_pr * T_pr_re_rompido_pr
            )
            ...
            R_rompido_pr_A = abs(
                forca_complexa(F_re_rompido_pr_A, ang_re) + 
                forca_complexa(F_vante_rompido_pr_A, ang_vante)
            )
            R_rompido_pr_B = abs(
                forca_complexa(F_re_rompido_pr_B, ang_re) + 
                forca_complexa(F_vante_rompido_pr_B, ang_vante)
            )
            
            # Escolhe o pior cenário
            if R_rompido_pr_A >= R_rompido_pr_B:
                pior_cenario_pr = "Rompimento do PR no ré"
                F_re_rompido_pr_final = F_re_rompido_pr_A
                F_vante_rompido_pr_final = F_vante_rompido_pr_A
            else:
                pior_cenario_pr = "Rompimento do PR no vante"
                F_re_rompido_pr_final = F_re_rompido_pr_B
                F_vante_rompido_pr_final = F_vante_rompido_pr_B
            
            # S_arrasto base para condutores (sempre presentes)
            S_arrasto_condutores = 0
            for i in range(len(K_fases)):
                if i < 3:
                    S_arrasto_condutores += D1 * K_fases[i] * cabos_por_fase
                else:
                    if is_duplo:
                        S_arrasto_condutores += D2 * K_fases[i] * cabos_por_fase
            
            # Adiciona o PR remanescente conforme o pior cenário
            if pior_cenario_pr == "Rompimento do PR no ré":
                # PR rompeu no ré → apenas PR do vante continua
                S_arrasto_pr_remanescente = D_pr * K_pr
            else:
                # PR rompeu no vante → apenas PR do ré continua
                S_arrasto_pr_remanescente = D_pr * K_pr
            
            # Área de arrasto total = condutores + PR remanescente
            S_arrasto_total = S_arrasto_condutores + S_arrasto_pr_remanescente
            
            # Ângulo crítico do vento com o PR remanescente
            ang_vento_pr_rompido_critico, _ = encontrar_angulo_critico_vento(
                F_re_rompido_pr_final, F_vante_rompido_pr_final, ang_re, ang_vante,
                S_arrasto_total, pressao_vento_reduzida, vao_re, vao_vante, F_vento_poste_reduzida
            )
            
            # Força de arrasto nos cabos com o PR remanescente
            seno_re_pr = abs(np.sin(np.radians(ang_vento_pr_rompido_critico - ang_re)))
            seno_vante_pr = abs(np.sin(np.radians(ang_vento_pr_rompido_critico - ang_vante)))
            F_vento_cabos_pr_rompido_mag = (
                (pressao_vento_reduzida / 2) * 
                (vao_re * seno_re_pr + vao_vante * seno_vante_pr) * 
                S_arrasto_total
            )
            F_vento_pr_rompido_mag = F_vento_cabos_pr_rompido_mag + F_vento_poste_reduzida
            
            # Resultante total com vento
            F_re_pr_rompido_vec = forca_complexa(F_re_rompido_pr_final, ang_re)
            F_vante_pr_rompido_vec = forca_complexa(F_vante_rompido_pr_final, ang_vante)
            F_vento_pr_rompido_vec = forca_complexa(F_vento_pr_rompido_mag, ang_vento_pr_rompido_critico)
            R_pr_rompido_vec = F_re_pr_rompido_vec + F_vante_pr_rompido_vec + F_vento_pr_rompido_vec
            R_pr_rompido = abs(R_pr_rompido_vec)
            theta_pr_rompido = np.degrees(np.angle(R_pr_rompido_vec))
            ...
            # Se não tem PR, define valores nulos
            R_pr_rompido_daN = 0
            poste_pr_rompido = 0
            F_re_rompido_pr_final = F_vante_rompido_pr_final = F_vento_pr_rompido_mag = R_pr_rompido = 0
            theta_pr_rompido = 0
            fator_pr_rompido = 1.0
            R_pr_rompido_corrigido = 0
            pior_cenario_pr = ""
        ...
        # Adiciona PR ROMPIDO se existir
        if tem_pr:
            dados_tabela.append({
                'Hipótese': 'PR ROMPIDO', 
                'Tração Ré (kgf)': f"{F_re_rompido_pr_final:.1f}", 
                'Tração Vante (kgf)': f"{F_vante_rompido_pr_final:.1f}", 
                'Força Vento (kgf)': f"{F_vento_pr_rompido_mag:.1f}", 
                'Resultante (kgf)': f"{R_pr_rompido:.1f}", 
        ...
        with st.expander("🌬️ VENTO MÁXIMO", expanded=False):
            ...
            st.text(f"   Força de vento no poste: {F_vento_poste_normal:.1f} kgf")
            st.text(f"   Trações (cabos): Ré = {F_re_vento_cabos:.1f} kgf | Vante = {F_vante_vento_cabos:.1f} kgf")
            st.text(f"   Força de vento total: {F_vento_mag:.1f} kgf")
        ...
        with st.expander("💔 CABO ROMPIDO", expanded=False):
            ...
            st.text(f"   Força de vento no poste: {F_vento_poste_reduzida:.1f} kgf")
        ...
        # Relatório do PR Rompido (se existir)
        if tem_pr:
            with st.expander("⚡ CABO PARA-RAIOS ROMPIDO", expanded=False):
                st.text(f"   Pressão vento reduzida: {pressao_vento_reduzida} kgf/m²")
                st.text(f"   Peso composto do PR: {peso_composto_pr_red:.4f} kgf/m")
                st.text(f"   Tração PR com vento: Ré = {T_pr_re_rompido_pr:.1f} kgf | Vante = {T_pr_vante_rompido_pr:.1f} kgf")
                st.text(f"   Resultante sem vento:")
                st.text(f"      Cenário A (rompe ré): {R_rompido_pr_A:.1f} kgf")
                st.text(f"      Cenário B (rompe vante): {R_rompido_pr_B:.1f} kgf")
                st.text(f"   Pior cenário: {pior_cenario_pr}")
                st.text(f"   Trações (pior cenário): Ré = {F_re_rompido_pr_final:.1f} kgf | Vante = {F_vante_rompido_pr_final:.1f} kgf")
                st.text(f"   Ângulo crítico do vento: {ang_vento_pr_rompido_critico:.1f}°")
                st.text(f"   Força de vento no poste: {F_vento_poste_reduzida:.1f} kgf")
                st.text(f"   Força de arrasto nos cabos: {F_vento_cabos_pr_rompido_mag:.1f} kgf")
                st.text(f"   Força de vento total: {F_vento_pr_rompido_mag:.1f} kgf")
                st.text(f"   Resultante total com vento: {R_pr_rompido:.1f} kgf")
                st.text(f"   Resultante final em daN: {R_pr_rompido_daN:.1f} daN")
        ...
        with col_conc1:
            ...
            if tem_pr:
                st.metric("PR ROMPIDO", f"{R_pr_rompido_daN:.1f} daN", f"Poste {poste_pr_rompido} daN")
        with col_conc2:
        ...
        if R_eds_daN > 4000 or R_min_daN > 4000 or R_vento_daN > 4000 or R_rompido_daN > 4000 or (tem_pr and R_pr_rompido_daN > 4000):
        ...
        # Determinar quantas abas terá
        num_tabs = 4 + (1 if tem_pr else 0)
        tab_titles = ["EDS", "TEMP MÍNIMA", "VENTO MÁXIMO", "CABO ROMPIDO"]
        if tem_pr:
            tab_titles.append("PR ROMPIDO")
        ...
        # Aba do PR Rompido (se existir)
        if tem_pr:
            with tabs[4]:
                fig5 = plotar_diagrama(
                    F_re_rompido_pr_final, F_vante_rompido_pr_final, F_vento_pr_rompido_mag, R_pr_rompido,
                    ang_re, ang_vante, ang_vento_pr_rompido_critico, theta_pr_rompido,
                    tipo_poste, f"PR ROMPIDO - Resultante: {R_pr_rompido_daN:.1f} daN",
                    theta_face if tipo_poste == 'Duplo T' else None,
                    None,
                    fator_pr_rompido if tipo_poste == 'Duplo T' else None
                )
                st.pyplot(fig5)
                plt.close(fig5)
