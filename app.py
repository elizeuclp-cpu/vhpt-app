...
with col1:
    ...
    if tem_estruturas:
        ...
        estrutura_escolhida = st.selectbox("Selecionar estrutura", [""] + nomes_estruturas, index=0)
        
        # Botão para aplicar a estrutura selecionada
        if st.button("📌 Aplicar estrutura selecionada", use_container_width=True):
            if estrutura_escolhida:
                estrutura_atual = next((e for e in estruturas if e['nome'] == estrutura_escolhida), None)
                if estrutura_atual:
                    st.session_state.valores_iniciais = {
                    ...
                    st.success(f"✅ Estrutura '{estrutura_escolhida}' aplicada!")
                    st.rerun()
            else:
                st.warning("⚠️ Selecione uma estrutura primeiro.")
    
    # Valores atuais (podem vir da estrutura ou do padrão)
    tipo_poste_default = st.session_state.valores_iniciais['tipo_poste']
    fixacao_default = st.session_state.valores_iniciais['fixacao']
    locacao_default = st.session_state.valores_iniciais['locacao']
    num_circuitos_default = st.session_state.valores_iniciais['num_circuitos']
    tem_pr_default = st.session_state.valores_iniciais['tem_pr']
    dist_f1_default = st.session_state.valores_iniciais['dist_f1']
    dist_f2_default = st.session_state.valores_iniciais['dist_f2']
    dist_f3_default = st.session_state.valores_iniciais['dist_f3']
    dist_f4_default = st.session_state.valores_iniciais['dist_f4']
    dist_f5_default = st.session_state.valores_iniciais['dist_f5']
    dist_f6_default = st.session_state.valores_iniciais['dist_f6']
    dist_pr_default = st.session_state.valores_iniciais['dist_pr']
