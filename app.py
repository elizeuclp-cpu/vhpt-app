...
# Inicializar session_state para armazenar valores padrão
...
if "estrutura_atual_nome" not in st.session_state:
    st.session_state.estrutura_atual_nome = ""
...
with col1:
    ...
    # Seletor de estruturas (se arquivo existir)
    if tem_estruturas:
        ...
        # Quando uma nova estrutura é selecionada, carregar seus valores
        if estrutura_selecionada and estrutura_selecionada != st.session_state.estrutura_atual_nome:
            ...
            if estrutura_atual:
                st.session_state.valores_estrutura = {
                    ...
                    'num_circuitos': estrutura_atual['circuito'],
                ...
                st.session_state.estrutura_atual_nome = estrutura_selecionada
                st.rerun()
        
        # Botão para reaplicar a estrutura atual (caso o usuário tenha editado e queira voltar)
        if estrutura_selecionada:
            if st.button("↩️ Reaplicar valores da estrutura", use_container_width=True):
                estrutura_atual = next((e for e in estruturas if e['nome'] == estrutura_selecionada), None)
                if estrutura_atual:
                    st.session_state.valores_estrutura = {
                        'tipo_poste': estrutura_atual['tipo_poste'],
                    ...
                    st.success(f"✅ Valores da estrutura '{estrutura_selecionada}' reaplicados!")
                    st.rerun()
    
    # Valores atuais (da estrutura selecionada ou padrão)
    tipo_poste_default = st.session_state.valores_estrutura['tipo_poste']
    fixacao_default = st.session_state.valores_estrutura['fixacao']
    locacao_default = st.session_state.valores_estrutura['locacao']
    num_circuitos_default = st.session_state.valores_estrutura['num_circuitos']
    tem_pr_default = st.session_state.valores_estrutura['tem_pr']
    dist_f1_default = st.session_state.valores_estrutura['dist_f1']
    dist_f2_default = st.session_state.valores_estrutura['dist_f2']
    dist_f3_default = st.session_state.valores_estrutura['dist_f3']
    dist_f4_default = st.session_state.valores_estrutura['dist_f4']
    dist_f5_default = st.session_state.valores_estrutura['dist_f5']
    dist_f6_default = st.session_state.valores_estrutura['dist_f6']
    dist_pr_default = st.session_state.valores_estrutura['dist_pr']
    
    # INPUTS
    tipo_poste = st.selectbox("Tipo de poste", ["Circular", "Duplo T"], index=0 if tipo_poste_default == "Circular" else 1)
    fixacao = st.selectbox("Tipo de fixação", ["suspensao", "ancoragem"], index=0 if fixacao_default == "suspensao" else 1, 
                           format_func=lambda x: "Suspensão" if x == "suspensao" else "Ancoragem")
    
    if fixacao == "suspensao":
        locacao = st.selectbox("Locação (Duplo T)", ["L1", "L2"], index=0 if locacao_default == "L1" else 1)
        deflexao = 0
        st.info("⚠️ Em suspensão, o ângulo de deflexão é zero (estrutura em alinhamento)")
    else:
        locacao = st.selectbox("Locação (Duplo T)", ["L3", "L4", "L5"], index=0 if locacao_default == "L3" else 1 if locacao_default == "L4" else 2)
    ...
    num_circuitos = st.selectbox("Circuitos", ["Circuito Simples", "Circuito Duplo"], index=0 if num_circuitos_default == "Circuito Simples" else 1)
...
if st.button("🔍 Calcular Esforço no Poste", type="primary", use_container_width=True):
    try:
        ...
        # ===================================================
        # EXIBIR RESULTADOS (EXATAMENTE IGUAL AO ORIGINAL)
        # ===================================================
        st.markdown("---")
