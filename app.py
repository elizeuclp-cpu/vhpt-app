...
with col1:
    ...
    if tem_estruturas:
        ...
        # Botão para carregar a estrutura selecionada
        if st.button("📥 Carregar estrutura selecionada", use_container_width=True):
            if estrutura_selecionada:
                estrutura_atual = next((e for e in estruturas if e['nome'] == estrutura_selecionada), None)
                if estrutura_atual:
                    # Salvar no session_state para usar nos campos
                    st.session_state.carregar_estrutura = {
                    ...
                    st.success(f"✅ Estrutura '{estrutura_selecionada}' carregada!")
                    st.rerun()
            else:
                st.warning("⚠️ Selecione uma estrutura primeiro.")
    
    # Valores padrão (vêm da estrutura carregada ou padrão)
    if 'carregar_estrutura' in st.session_state:
        tipo_poste_default = st.session_state.carregar_estrutura['tipo_poste']
        fixacao_default = st.session_state.carregar_estrutura['fixacao']
        locacao_default = st.session_state.carregar_estrutura['locacao']
        num_circuitos_default = st.session_state.carregar_estrutura['num_circuitos']
        tem_pr_default = st.session_state.carregar_estrutura['tem_pr']
        dist_f1_default = st.session_state.carregar_estrutura['dist_f1']
        dist_f2_default = st.session_state.carregar_estrutura['dist_f2']
        dist_f3_default = st.session_state.carregar_estrutura['dist_f3']
        dist_f4_default = st.session_state.carregar_estrutura['dist_f4']
        dist_f5_default = st.session_state.carregar_estrutura['dist_f5']
        dist_f6_default = st.session_state.carregar_estrutura['dist_f6']
        dist_pr_default = st.session_state.carregar_estrutura['dist_pr']
    else:
        tipo_poste_default = "Duplo T"
        fixacao_default = "ancoragem"
        locacao_default = "L3"
        num_circuitos_default = "Circuito Simples"
