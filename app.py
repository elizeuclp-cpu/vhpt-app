...
with col1:
    ...
    if tem_estruturas:
        ...
        estrutura_escolhida = st.selectbox("Selecionar estrutura (opcional)", [""] + nomes_estruturas, index=0)
        ...
        if estrutura_escolhida:
            ...
            if estrutura_atual:
                ...
                is_duplo_flag = (estrutura_atual['circuito'] == "Circuito Duplo")
                tem_pr_flag = estrutura_atual['tem_pr']
                dist_f1_val = estrutura_atual['dist_f1']
                dist_f2_val = estrutura_atual['dist_f2']
                dist_f3_val = estrutura_atual['dist_f3']
                dist_f4_val = estrutura_atual['dist_f4']
                dist_f5_val = estrutura_atual['dist_f5']
                dist_f6_val = estrutura_atual['dist_f6']
                dist_pr_val = estrutura_atual['dist_pr']
            else:
                tipo_poste = "Duplo T"
                fixacao = "ancoragem"
                locacao = "L3"
    ...
    # INPUTS (exatamente iguais ao original)
    tipo_poste = st.selectbox("Tipo de poste", ["Circular", "Duplo T"], index=0 if tipo_poste == "Circular" else 1)
    fixacao = st.selectbox("Tipo de fixação", ["suspensao", "ancoragem"], index=0 if fixacao == "suspensao" else 1, 
                           format_func=lambda x: "Suspensão" if x == "suspensao" else "Ancoragem")
    
    if fixacao == "suspensao":
        locacao = st.selectbox("Locação (Duplo T)", ["L1", "L2"], index=0 if locacao == "L1" else 1)
        deflexao = 0
        st.info("⚠️ Em suspensão, o ângulo de deflexão é zero (estrutura em alinhamento)")
    else:
        locacao = st.selectbox("Locação (Duplo T)", ["L3", "L4", "L5"], index=0 if locacao == "L3" else 1 if locacao == "L4" else 2)
...
with col2:
    ...
    with col_f1:
        dist_f1 = st.number_input("Fase 1 (m)", value=dist_f1_val, step=0.05)
    with col_f2:
        dist_f2 = st.number_input("Fase 2 (m)", value=dist_f2_val, step=0.05)
    with col_f3:
        dist_f3 = st.number_input("Fase 3 (m)", value=dist_f3_val, step=0.05)
    
    if is_duplo:
        st.markdown("**Circuito 2:**")
        col_f4, col_f5, col_f6 = st.columns(3)
        with col_f4:
            dist_f4 = st.number_input("Fase 4 (m)", value=dist_f4_val, step=0.05)
        with col_f5:
            dist_f5 = st.number_input("Fase 5 (m)", value=dist_f5_val, step=0.05)
        with col_f6:
            dist_f6 = st.number_input("Fase 6 (m)", value=dist_f6_val, step=0.05)
    ...
    tem_pr = st.checkbox("Possui cabo para-raios?", value=tem_pr_flag)
    
    if tem_pr:
        tipo_pr = st.selectbox("Tipo de PR", ["Tipo A", "Tipo B", "Tipo C"], index=0)
        dist_pr = st.number_input("Distância PR ao topo (m)", value=dist_pr_val, step=0.05)
